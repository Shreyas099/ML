...
class LSTMModel:
    ...
    def predict_residuals(self, last_residuals: pd.Series, steps: int,
        ...
        if expected_features > 0:
            ...
            # Reshape last_seq to include feature placeholders for initial sequence
            # We need to create a combined sequence with dummy features
            dummy_features = np.zeros((self.sequence_length, expected_features))
            last_seq = np.column_stack([last_seq.reshape(-1, 1), dummy_features])
        else:
